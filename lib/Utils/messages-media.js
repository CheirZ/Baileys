...
export const getHttpStream = async (url, options = {}) => {
    ...
    try {
        ...
        throw new Boom(`Failed to fetch stream from ${url}`, { statusCode: error?.response?.status, data: { url } });
    }
};
...
const uploadMedia = async ({ url, filePath, headers, timeoutMs, agent }) => {
    const { default: axios } = await import('axios');
    ...
    return async (filePath, { mediaType, fileEncSha256B64, timeoutMs }) => {
        ...
        for (let attempt = 0; attempt < maxRetries && !urls; attempt++) {
            ...
            for (const { hostname, maxContentLengthBytes } of hosts) {
                ...
                try {
                    const fileStats = await fs.stat(filePath);
                    if (maxContentLengthBytes && fileStats.size > maxContentLengthBytes) {
                        throw new Boom('Body too large for host', { statusCode: 413, data: { hostname, size: fileStats.size, maxContentLengthBytes } });
                    }
