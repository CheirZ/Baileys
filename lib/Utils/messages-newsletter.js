...
export function makeNewsletterUtils(conn) {
    ...
    async function sendNewsletterImage(jid, image, options = {}) {
        return send(jid, { image, caption: options.caption, mimetype: options.mimetype, jpegThumbnail: options.jpegThumbnail }, options);
    }
    async function sendNewsletterVideo(jid, video, options = {}) {
        return send(jid, { video, caption: options.caption, mimetype: options.mimetype, gifPlayback: options.gifPlayback }, options);
    }
    async function sendNewsletterPtv(jid, video, options = {}) {
        return send(jid, { video, ptv: true, mimetype: options.mimetype || 'video/mp4' }, options);
    }
    async function sendNewsletterAudio(jid, audio, options = {}) {
        return send(jid, { audio, mimetype: options.mimetype, seconds: options.seconds, ptt: options.ptt }, options);
    }
    ...
    async function sendNewsletterSticker(jid, sticker, options = {}) {
        return send(jid, { sticker, isAnimated: options.isAnimated }, options);
    }
    async function sendNewsletterButtons(jid, params, options = {}) {
        ...
        return send(jid, { interactiveMessage }, options);
    }
